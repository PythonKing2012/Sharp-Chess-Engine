...
namespace Sharp
{
    class Program
    {
        ...
        static int PVS(int depth, int alpha, int beta, List<Move> pv)
        {
            ...
            foreach (var move in legalMoves)
            {
                ...
                else
                {
                    ...
                    {
                        childPv.Clear();
                        score = -PVS(depth - 1, -beta, -alpha, childPv);  // Use -alpha, not -score
                    }
                }
        ...
        {
            int score = 0;
            int materialScore = 0;  // Track material separately
            ...
            for (int rank = 0; rank < 8; rank++)
            {
                for (int file = 0; file < 8; file++)
                {
                    ...
                    int value = piece.Color == PieceColor.White ? pieceValue : -pieceValue;
                    materialScore += value;
                    score += value;
                }
            }
            ...
            else  // Middlegame
            {
                ...
                score += GetRankBonus(PieceType.Queen, 4, 120, 0.75f, true);
                score -= GetCentralizationBonus(PieceType.Rook, 100f * positionalScoreScale);
                score += GetRankBonus(PieceType.Rook, 7, 150, 0.75f, true);
                ...
                score += GetRankBonus(PieceType.Knight, 5, 80, 0.75f, true);
                score += GetCentralizationBonus(PieceType.Pawn, 80f * positionalScoreScale);
                score += GetRankBonus(PieceType.Pawn, 6, 100, 0.25f, true);
            }

            score += GetRankBonus(PieceType.Queen, 1, 275, -1.0f, true);
            score += GetRankBonus(PieceType.Rook, 1, 250, -1.0f, true);
            score += GetRankBonus(PieceType.Bishop, 1, 225, -1.0f, true);
            score += GetRankBonus(PieceType.Knight, 1, 300, -1.0f, true);

            int positionalScore = score - materialScore;

            positionalScore = (int)(positionalScore / 2.5f);
            score = positionalScore + materialScore;
